...
9. File system (read, write info). File partioning - space division. In unix, / is refered to as root dir.
> eg, /banana = root/banana.
Partitions can be nested on root dir.
 
10. IPC (Interprocess Communication) - any mechanism(files, pipes, sockets, signals, shared memory) that facilitates communication b/n processes 
